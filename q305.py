...
class DSU(object):
    ...
    def place(self, r, c):
        ...
        originalCC = set()
        # need to calculate and record parents before actually connecting them, because during the
        # connection the parents might be changed.
        for r_, c_ in [(r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1)]:
            if 0 <= r_ <= self.rows - 1 and 0 <= c_ <= self.cols - 1 and (r_, c_) in self.placed:
                originalCC.add(self.query(self.getIndex((r_, c_))))
        for r_, c_ in [(r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1)]:
            if 0 <= r_ <= self.rows - 1 and 0 <= c_ <= self.cols - 1 and (r_, c_) in self.placed:
                self.connect((r, c), (r_, c_))
        self.count -= len(originalCC)
        self.count += 1
        self.placed.add((r,c))
        return self.count
    ...
    def connect(self, x, y):
        x, y = self.getIndex(x), self.getIndex(y)
